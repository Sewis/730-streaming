...
{
  public static boolean USE_RATETIMITER = true;
  public static double RATELIMITER_MBPS = 5;
  
  protected final Logger logger = LoggerFactory.getLogger(this.getClass());
  
  private static final int DEFAULT_BUFFER_SIZE = 20480; // 20KB in Bytes.
  private static final long DEFAULT_EXPIRE_TIME = 604800000L; // 1 week in Milliseconds.
  ...
  private static class Range
  {
    ...
    private static void copy(InputStream input, OutputStream output, long inputSize, long start, long length) throws IOException
    {
      ...
      final RateLimiter rateLimiter = RateLimiter.create(RATELIMITER_MBPS * 1000 * 1000.0);
      ...
      else
      {
        ...
        while ((read = input.read(buffer)) > 0)
        {
          ...
          {
            rateLimiter.acquire((int)toRead + read);
